...
// MARK: - Architecture regression tests (DNSService)

extension DNSServiceTests {
    /// After cache TTL expires, FakeIPPool still owns the mapping — re-query must reuse the same fake IP.
    /// Documents cache↔pool lifetime coupling (pool never recycles on sweep).
    func testExpiredCacheStillReusesSameFakeIP() {
        let service = DNSService(
            eventLoop: loop,
            ttl: 1,
            upstreamTimeout: .milliseconds(200),
            primaryUpstream: .init(host: "192.0.2.1", port: 53),
            secondaryUpstreams: []
        )
        let domain = "reuse-after-expire.example"
        let queryData = DNSMessageBuilder.buildAQuery(domain: domain)

        let exp1 = expectation(description: "first assign")
        let exp2 = expectation(description: "after expire")
        var firstIPBox: IPv4Address?

        service.handleDNSPayload(FBDataPacketBuffer(queryData), loop).whenComplete { result in
            if case let .success(data?) = result {
                firstIPBox = self.extractFirstIPv4(from: data)
            }
            exp1.fulfill()
        }
        wait(for: [exp1], timeout: 2.0)
        guard let firstIP = firstIPBox else {
            XCTFail("no fake IP from first query")
            return
        }

        loop.scheduleTask(in: .milliseconds(1200)) {
            service.handleDNSPayload(FBDataPacketBuffer(queryData), self.loop).whenComplete {
                result in
                if case let .success(data?) = result {
                    let second = self.extractFirstIPv4(from: data)
                    XCTAssertEqual(second, firstIP, "FakeIP must survive cache expiry")
                } else {
                    XCTFail("second query should succeed")
                }
                exp2.fulfill()
            }
        }
    ...
    /// Dial decision after A assign: fake IP without realIPs → dialHost = domain.
    /// After cache entry expires, reverse mapping still works (pool not cleared).
    func testDialDecisionSurvivesCacheExpiry() {
        let service = DNSService(
            eventLoop: loop,
            ttl: 1,
            upstreamTimeout: .milliseconds(200),
        ...
        let domain = "dial-survive.example"
        let queryData = DNSMessageBuilder.buildAQuery(domain: domain)

        let exp1 = expectation(description: "assign")
        let exp2 = expectation(description: "dial after expire")
        var fakeIPBox: IPv4Address?

        service.handleDNSPayload(FBDataPacketBuffer(queryData), loop).whenComplete { result in
            if case let .success(data?) = result {
                fakeIPBox = self.extractFirstIPv4(from: data)
            }
            exp1.fulfill()
        }
        wait(for: [exp1], timeout: 2.0)
        guard let fakeIP = fakeIPBox else {
            XCTFail("no fake IP")
            return
        }

        loop.scheduleTask(in: .milliseconds(1200)) {
            service.resolveDialDecision(fakeIP, self.loop).whenComplete { result in
                switch result {
                case let .success(decision):
                    XCTAssertTrue(decision.fromFakeIP)
                    XCTAssertEqual(decision.dialHost, domain)
                case let .failure(error):
                    XCTFail("dial failed: \(error)")
                }
                exp2.fulfill()
            }
        }
    ...
    /// Unknown qtypes (not in DNSType) fail fast sniff → slow path → upstream (nil on timeout).
    func testUnknownQTypeStillForwardsViaSlowPath() {
        let service = DNSService(
            eventLoop: loop,
            ttl: 300,
            upstreamTimeout: .milliseconds(100),
            primaryUpstream: .init(host: "192.0.2.1", port: 53),
            secondaryUpstreams: []
        )
        let exp = expectation(description: "mx slow path")

        var writer = FBPacketBufferWriter()
        writer.writeUInt16(0x5555)
        writer.writeUInt16(0x0100)
        writer.writeUInt16(1)
        writer.writeUInt16(0)
        writer.writeUInt16(0)
        writer.writeUInt16(0)
        writer.name("mail.slowpath.example")
        writer.writeUInt16(15) // MX — not in DNSType
        writer.writeUInt16(1)

        service.handleDNSPayload(FBDataPacketBuffer(writer.data), loop).whenComplete { result in
            // Upstream timeout → nil; must not crash or FORMERR
            if case let .success(data) = result {
                XCTAssertNil(data)
            } else {
                XCTFail("should succeed with nil, not fail the future")
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 2.0)
