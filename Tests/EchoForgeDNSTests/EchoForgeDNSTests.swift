...
final class FakeIPPoolTests: XCTestCase {
    func testAssignAndReverseLookup() throws {
...
final class DNSCacheTests: XCTestCase {
    func testInsertAndLookup() throws {
        ...
        let entry = DNSCacheEntry(key: key, fakeIP: IPv4Address("198.18.0.2")!, expireAt: .now() + .seconds(10), realIPs: [IPv4Address("1.2.3.4")!])
        try loop.submit {
            cache.insert(entry)
    ...
    func testSweepExpired() throws {
        let group = MultiThreadedEventLoopGroup(numberOfThreads: 1)
        defer { try? group.syncShutdownGracefully() }
        let loop = group.next()
        let cache = DNSCache(eventLoop: loop)
        let key = DNSCacheKey(domain: "b.com", type: .a)
        let entry = DNSCacheEntry(key: key, fakeIP: IPv4Address("198.18.0.3")!, expireAt: .now() - .seconds(1), realIPs: nil)
        try loop.submit {
            cache.insert(entry)
